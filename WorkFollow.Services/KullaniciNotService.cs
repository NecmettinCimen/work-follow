...
public class KullaniciNotService : IKullaniciNotService
{
    ...
    {
        var tblnotid = _tblNotService.Insert(tblNot);
        ...
        const string Sql =
            "insert into public.kullanicinot (kullaniciid, notid, etkinlikid) values (@kullaniciid, @notid, @etkinlikid)";
    ...
    {
        const string Sql =
            @"select t.id, t.konu, t.aciklama from public.tblnot t, public.kullanicinot kt where t.id = kt.notid and kt.kullaniciid =@kullaniciid and kt.etkinlikid = @etkinlikid";
